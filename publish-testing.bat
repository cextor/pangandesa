...
if not exist package.json (
    ...
    echo 1. Klik kanan file "publish-testing.bat" di folder proyek.
...
echo ===================================================
echo   PanganDesa Deployer - Branch: testing (QA)
echo ===================================================
echo.
...
echo [4/4] Menginisialisasi Git di folder dist dan melakukan push ke branch 'testing'...
cd dist
git init -q
git remote add origin !REMOTELINK!
git checkout -b testing -q
git add .
git commit -m "Testing QA update: %date% %time%" -q
...
if %errorlevel% neq 0 (
    echo.
    echo [ERROR] Gagal melakukan push ke branch 'testing'.
    echo Pastikan kredensial GitHub atau SSH Key Anda sudah aktif.
) else (
    echo.
    echo ===================================================
    echo   SUKSES! File dist berhasil didorong ke branch: testing
    echo ===================================================
    echo.
    echo SEKARANG DI hPANEL HOSTINGER (UNTUK QA.PANGANDESA.COM):
    echo 1. Hubungkan repositori Anda: !REMOTELINK!
    echo 2. Setel "Branch" yang akan di-deploy ke: testing
    echo 3. Arahkan "Direktori Instalasi" ke folder qa.pangandesa.com Anda (misal: /qa.pangandesa.com)
