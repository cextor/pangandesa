    ...
    <div className="bg-white min-h-screen lg:min-h-0 lg:rounded-[40px] overflow-hidden flex flex-col">
      <div className="p-8 border-b border-slate-50">
        <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight flex items-center gap-3">
          <FileText className="text-brand-600" /> Invoice Pembayaran
        </h2>
        <p className="text-slate-500 font-medium">Selesaikan pembayaran Down Payment untuk mengamankan pre-order Anda.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
        ...
        <div className="bg-slate-50 rounded-[32px] p-8 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-200">
            ...
            <span className="text-sm font-bold text-slate-800">#{order.id.toUpperCase()}</span>
          </div>
            ...
            {order.items.map((item, i) => (
              <div key={i} className="flex justify-between items-center">
                <div className="flex gap-4 items-center">
                  <div className="w-12 h-12 rounded-xl border border-white overflow-hidden shadow-sm">
          ...
          <div className="pt-6 border-t border-slate-200 space-y-4">
            <div className="flex justify-between items-center opacity-60">
              ...
              <span className="text-sm font-bold text-slate-800">Rp {order.totalAmount.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between items-center p-6 bg-brand-50 rounded-2xl border border-brand-100">
              <div>
                ...
                <p className="text-2xl font-black text-brand-900 tracking-tight">
                  Rp {(order.status === 'WAITING_PAYMENT_DP' ? order.dpAmount : order.remainingAmount).toLocaleString('id-ID')}
                </p>
              </div>
              <ClipboardCheck size={32} className="text-brand-300" />
            </div>
             <p className="text-[10px] text-center text-slate-400 font-medium whitespace-pre-line">
        ...
        <button 
          ...
          className="w-full bg-brand-600 text-white py-6 rounded-[32px] font-black uppercase tracking-widest shadow-xl shadow-brand-600/20 active:scale-95 transition-all flex items-center justify-center gap-3 hover:bg-brand-700"
        >
          Konfirmasi & Bayar DP <ArrowRight size={20} />
        </button>
      </div>
    </div>
  );
