...
export default function SellerProfilePage({ onBack }: SellerProfileProps) {
  ...
  const [picName, setPicName] = useState(user?.pic_name || '');
  ...
  const defaultAvatar = 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=400&auto=format&fit=crop';
  const sellerAvatar = user?.avatar || defaultAvatar;
    ...
    <div className="flex-1 bg-slate-50 overflow-y-auto custom-scrollbar p-4 md:p-8 lg:p-10">
      ...
      <div className="max-w-4xl mx-auto space-y-6 md:space-y-10 pb-20">
        ...
        <div className="flex items-center gap-4 md:gap-6">
           <button 
             onClick={onBack}
             className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white border border-slate-100 rounded-xl md:rounded-2xl text-slate-400 hover:text-slate-800 hover:shadow-md transition-all active:scale-95 shadow-sm cursor-pointer"
           >
           ...
           <div className="relative pt-12 md:pt-16 px-6 md:px-10 pb-8 md:pb-10 flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8">
              ...
              <div className="flex-1 text-center md:text-left pb-2 md:pb-4">
                  <div className="flex flex-col md:flex-row items-center gap-3 mb-3 md:mb-2">
                     <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight leading-tight">{user?.name || 'Pak Joko Lembang'}</h2>
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 md:gap-4 text-slate-400">
                     <div className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold uppercase tracking-widest">
                        <Store size={14} className="text-brand-500" /> {user?.company_name || 'Toko Berkah Tani'}
                     </div>
              ...
              <div className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[48px] border border-slate-100 shadow-sm space-y-8 md:space-y-10">
                  ...
                  <div className="space-y-5 md:space-y-6">
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div className="space-y-1.5 md:space-y-2">
                           <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama Toko / Kelompok Tani</label>
                           <div className="relative group">
                  ...
                  <div className="space-y-5 md:space-y-6">
                     <h3 className="text-[9px] md:text-[10px] font-black text-brand-600 uppercase tracking-[0.2em] px-1 flex items-center gap-3">
                        <CreditCard size={14} /> Informasi Rekening Bank
                     </h3>
                     
                     <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                        <div className="space-y-1.5 md:space-y-2">
                           <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama Bank</label>
                           <input 
                             type="text" 
                             defaultValue="BANK BRI"
                             className="w-full bg-slate-50 border border-slate-100 rounded-xl md:rounded-2xl p-4 md:p-5 text-xs md:text-sm font-bold focus:ring-4 focus:ring-brand-500/5 focus:bg-white focus:border-brand-200 outline-none transition-all shadow-inner"
                           />
                        </div>
                        <div className="space-y-1.5 md:space-y-2 sm:col-span-2">
                           <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nomor Rekening</label>
                           <input 
                             type="text" 
                             value={bankAccount}
                     ...
                     <div className="space-y-1.5 md:space-y-2">
                        <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama Pemilik Rekening</label>
                        <input 
                          type="text" 
                          value={name}
                          onChange={(e) => setName(e.target.value)}
                  ...
                  <div className="space-y-5 md:space-y-6">
                     <h3 className="text-[9px] md:text-[10px] font-black text-brand-600 uppercase tracking-[0.2em] px-1 flex items-center gap-3">
                        <User size={14} /> PIC Admin Toko
                     </h3>
                     
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                        <div className="space-y-1.5 md:space-y-2">
                           <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama PIC Admin</label>
                           <input 
                             type="text" 
                             value={picName}
                        ...
                        <div className="space-y-1.5 md:space-y-2">
                           <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">WhatsApp PIC</label>
                           <input 
                             type="text" 
                             value={phone}
                             onChange={(e) => setPhone(e.target.value)}
                     ...
                     <div className="space-y-1.5 md:space-y-2">
                        <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Jabatan PIC</label>
                        <input 
                          type="text" 
                          defaultValue="Manajer Operasional & Tani"
                          className="w-full bg-slate-50 border border-slate-100 rounded-xl md:rounded-2xl p-4 md:p-5 text-xs md:text-sm font-bold focus:ring-4 focus:ring-brand-500/5 focus:bg-white focus:border-brand-200 outline-none transition-all shadow-inner"
                        />
                     </div>
                  </div>
                  ...
                  <div className="space-y-5 md:space-y-6">
                     ...
                     <div className="space-y-1.5 md:space-y-2">
                        <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Alamat Penjemputan / Pengiriman</label>
                        <div className="relative group">
                  ...
                  <div className="pt-4 flex flex-col sm:flex-row gap-3 md:gap-4">
                     ...
                     <button 
                       type="button"
                       onClick={onBack}
                       className="px-8 md:px-10 py-4 md:py-5 rounded-xl md:rounded-[24px] text-xs md:text-sm font-black uppercase tracking-widest text-slate-500 border border-slate-200 hover:bg-slate-50 transition-all active:scale-95 cursor-pointer"
                     >
                        Batal
                     </button>
                  </div>
