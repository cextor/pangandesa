        ...
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row lg:items-center justify-between gap-8">
           <div>
              <div className="flex items-center gap-3 mb-1">
                 ...
                 <h1 className="text-2xl font-black text-slate-800 font-display">Verifikasi Pembayaran</h1>
              </div>
              <p className="text-xs text-slate-500 font-medium">Verifikasi slip setoran Down Payment (30%) dan Pelunasan (70%) nasabah.</p>
           </div>
      ...
      <div className="flex-1 overflow-y-auto p-10 custom-scrollbar pb-32">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">Antrean Pembayaran Pending</h3>
            ...
            <table className="w-full text-left">
              <thead>
                 <tr className="border-b border-slate-50 bg-slate-50/50">
                    ...
                    <th className="p-8 text-[10px] font-black text-slate-400 uppercase tracking-widest pr-8 text-right">Aksi</th>
                 </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 font-display">
                 {filteredOrders.length > 0 ? filteredOrders.map((order) => (
                   <tr key={order.id} className="hover:bg-slate-50/30 transition-colors group">
                     ...
                     <td className="p-8 pr-8 text-right">
                        <div className="flex items-center justify-end gap-2">
                           <button 
                             onClick={() => onConfirmPayment(order.id, order.status === 'WAITING_ADMIN_DP' ? 'DP' : 'FINAL')}
                             className="bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all hover:bg-emerald-600 flex items-center gap-2 border-0 cursor-pointer"
                           >
                             <CheckCircle2 size={14} /> Verifikasi
                           </button>
                           <button 
                             onClick={() => navigate('/admin/transaksi-panen')}
                             className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-brand-50 hover:text-brand-600 transition-all flex items-center justify-center cursor-pointer border-0"
                             title="Diskusi Forum"
                           >
                             <MessageSquare size={16} />
                           </button>
                           <button className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-red-50 hover:text-red-500 transition-all border-0 cursor-pointer">
                             <XCircle size={18} />
                           </button>
                        </div>
                     </td>
